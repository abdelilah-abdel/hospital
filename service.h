#ifndef SERVICE_H_SERVICE_H
#define SERVICE_H_SERVICE_H
...
public:
    ...
    string get_codeservice(){return  code_service;}
...
bool service::verifier_existance_Inf(string code) {
    for (int i; i < nbr_infermier; i++) {
...
bool service::verifier_existance_Inf(service& S, string code) {
...
bool service::verifier_nbr_inf() const {
    if (nbr_infermier > 20) {
        return false;
    }
    else return true;
...
// 2
void service::inf_5exp(infirmier* tabinf) {
    for (int i; i < nbr_infermier; i++) {
...
//----------------------------------1
int  service::Age_service( string idservice) {
    int position, A, B, C;
...
int  service::Age_service(service* tabs, string idservice) {
    int position, A, B, C;
    for (int i; i < nbr_infermier; i++) {
...
bool  service::Supprimer_inf_service(string codeinf) {
...
void service::Modier_nom_service() {
...
void service::Modier_nom_service(service& S) {
...
service& service::operator =(service &S){
    service servicecopy ;
    servicecopy.code_service = S.code_service;
    servicecopy.nom_service =S.nom_service;
    servicecopy.Med_chef=S.Med_chef;
    servicecopy.secretaire=S.secretaire;
    servicecopy.nbr_infermier=S.nbr_infermier;
    servicecopy.date_lancement_S = S.date_lancement_S; // needs an operator surcharge ,
    servicecopy.Tabinfermiers=S.Tabinfermiers; // needs a surcharge or a methode
    return servicecopy;
}

bool service::modifier_nom_s(){
    cout<<"entrer le nouveau nom de service";
    cin>>nom_service;
    return true;
}
bool service::modifier_med_c(){
    Med_chef.saisir_Med();
    return true;
}